...
$cert | Export-Certificate -FilePath "$sslDir/branchloans.cer" -Type CERT -Force
...
# Generate a private key (this is a simplified version, in production use a proper key generation)
$privateKey = @"
-----BEGIN PRIVATE KEY-----
...
Write-Host "To trust the certificate, double-click on $sslDir/branchloans.cer and install it in the 'Trusted Root Certification Authorities' store."
